...
elif [[ -x ./build/cellCentered ]]; then
    SOLVER=./build/cellCentered
else
    echo "Error: cannot find ./build/staggerdeGrid or ./build/cellCentered" >&2
    exit 1
fi
...
# build/output/cellCentered_M%zu_N%zu_Rho.csv
# ------------------------------------------------------------
rho_file()
{
    local n=$1

    printf '%s/cellCentered_M%s_N%s_Rho.csv\n' \
        "$OUTDIR" "$M" "$n"
}
